(function () {
  ...
  // ========== ORIGINAL UI FUNCTIONS (keep as they were) ==========
  ...
  async function getAllNodes() {
    const data = await loadNodesFromJSON();
    const flat = [];
    for (const guide of (data.guides || [])) {
      for (const [nodeId, node] of Object.entries(guide.nodes || {})) {
        flat.push({
          ...
          nodeId,
          node,
          searchText: `${guide.title} ${node.text} ${node.help || ''} ${node.note || ''} ${node.action || ''} ${(node.choices || []).map(c => c.label + ' ' + (c.action || '')).join(' ')}`.toLowerCase()
        });
      }
    }
    return flat;
